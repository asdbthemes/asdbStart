...
class data_source {
       ...
       // $current_day = date('j');
        ...
        switch ($sort) {
            case 'featured':
                if (!empty($category_ids)) {
                    ...
                    foreach ($cat_id_array as &$cat_id) {
                        ...
                        if (empty($wp_query_args['category_name'])) {
                            $wp_query_args['category_name'] = $tmp_cat_obj->slug; //get by slug (we get the children categories too)
                        } else {
                            $wp_query_args['category_name'] .= ',' . $tmp_cat_obj->slug; //get by slug (we get the children categories too)
                        }
                        unset($tmp_cat_obj);
                ...
                $wp_query_args['cat'] = get_cat_ID( ot_get_option('featured-category') );
                break;
            case 'popular':
     ...
     * @param $homepage_loop_filter - the post loop filer metadata array
     * @param string $paged
     * @return array
     */
    static function metabox_to_args($homepage_loop_filter, $paged = '') {
        ...
        if (!empty($homepage_loop_filter['show_featured_posts'])) {
    ...
    static function hook_fix_offset_pagination($found_posts, $query) {
        ...
        return $found_posts - data_source::$fake_loop_offset;
    }
