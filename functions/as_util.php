...
class as_util {
    ...
    */
    static function read_once_theme_settings() {
        as_global::$as_options = get_option(as_THEME_OPTIONS_NAME);
    }
     ...
     *   implementation in @see as_panel_data_source::update_category
    ...
    static function get_category_option($category_id, $option_id) {
        if (isset(as_global::$as_options['category_options'][$category_id][$option_id])) {
            return as_global::$as_options['category_options'][$category_id][$option_id];
        } else {
            return '';
        }
    }
     ...
     * - We have no update method because the panel has it's own update
     *   implementation in @see as_panel_data_source::update_as_cpt
     * - the panel uses this function to read settings for specific categories
     * - it is used also in the entire theme
    ...
    static function get_ctp_option($custom_post_type, $option_id) {
        if (isset(as_global::$as_options['as_cpt'][$custom_post_type][$option_id])) {
            return as_global::$as_options['as_cpt'][$custom_post_type][$option_id];
        } else {
            return '';
        }
    }
     ...
     * - We have no update method because the panel has it's own update
     *   implementation in @see as_panel_data_source::update_as_taxonomy
     * - the panel uses this function to read settings for specific categories
     * - it is used also in the entire theme
     * @param $taxonomy_name
     * @param $option_id
    ...
    static function get_taxonomy_option($taxonomy_name, $option_id) {
        if (isset(as_global::$as_options['as_taxonomy'][$taxonomy_name][$option_id])) {
            return as_global::$as_options['as_taxonomy'][$taxonomy_name][$option_id];
        } else {
            return '';
        }
    }
     ...
     * @return string
     */
    static function get_as_ads($ad_position_id) {
        //print_r(as_global::$as_options);
        if (isset(as_global::$as_options['as_ads'][$ad_position_id])) {
            return as_global::$as_options['as_ads'];
        } else {
            return '';
        }
    }
    ...
    static function is_ad_spot_enabled($ad_spot_id) {
        if (isset(as_global::$as_options['as_ads'][$ad_spot_id]['ad_code'])) {
            return true;
        } else {
            return false;
        }
    ...
    static function get_option($optionName, $default_value = '') {
        //$theme_options = get_option(as_THEME_OPTIONS_NAME);

        if (!empty(as_global::$as_options[$optionName])) {
            return as_global::$as_options[$optionName];
        } else {
    ...
    //updates a theme option @todo sa updateze globala as_util::$as_options
    static function update_option($optionName, $newValue) {
        //as_global::$as_options = get_option(as_THEME_OPTIONS_NAME);

        as_global::$as_options[$optionName] = $newValue;



        update_option(as_THEME_OPTIONS_NAME, as_global::$as_options);
    }
    ...
    static function cut_title($cut_parms, $title) {
        ...
        $title = trim($title);
        $title = as_util::excerpt($title,$cut_parms['excerpt']);
    ...
    static function show_sidebar($template_id) {
        $tds_cur_sidebar = as_util::get_option('tds_' . $template_id . '_sidebar');
        if (!empty($tds_cur_sidebar)) {
            dynamic_sidebar($tds_cur_sidebar);
        } else {
            //show default
            if (!dynamic_sidebar(as_THEME_NAME . ' default')) {
                ?>
                <!-- .no sidebar -->
                <?php
            }
    ...
    static function get_image_attachment_data($post_id, $size = 'as_180x135', $count = 1 ) {//'thumbnail'
        $objMeta = array();
        $meta = '';// (stdClass)
        $args = array(
            'numberposts' => $count,
    ...
    /*  ----------------------------------------------------------------------------
        used by the css compiler in /includes/app/as_css_generator.php
     */
    ...
    /**
     * create $as_authors array in format id_author => display_name_author
    ...
    static function create_array_authors() {
        ...
        if (is_admin()) {
            ...
            $as_authors = array();
            $as_return_obj_authors = get_users('role=Administrator');

            $as_authors[' - No author filter - '] = '';
            foreach($as_return_obj_authors as $obj_autor){
                $auth_id = $obj_autor->ID;
                $auth_name = $obj_autor->display_name;

                $as_authors[$auth_name] = $auth_id;
            }

            self::$authors_array_cache = $as_authors;
    ...
    static function excerpt($post_content, $limit, $show_shortcodes = '') {
        ...
        //excerpt for letters
        if (as_util::get_option('tds_excerpts_type') == 'letters') {
     ...
     * @return mixed
     */
    static $as_category2id_array_walker_buffer = array();
    static function get_category2id_array($add_all_category = true) {
        ...
        if (empty(self::$as_category2id_array_walker_buffer)) {
            $categories = get_categories(array(
                'hide_empty' => 0,
                'number' => 1000
            ));

            $as_category2id_array_walker = new as_category2id_array_walker;
            $as_category2id_array_walker->walk($categories, 4);
            self::$as_category2id_array_walker_buffer = $as_category2id_array_walker->as_array_buffer;
        }
        ...
        if ($add_all_category === true) {
            ...
            return array_merge(
                $categories_buffer,
                self::$as_category2id_array_walker_buffer
            );
        } else {
            return self::$as_category2id_array_walker_buffer;
        }
    }
    ...
    static function vc_set_column_number($as_columns) {
        global $as_row_count, $as_column_count;
        $as_row_count = 1;
     ...
     * $as_row_count, $as_column_count are from the pagebuilder
     * @return int
     */
    static function vc_get_column_number() {
        global $as_row_count, $as_column_count, $post;

        //echo 'xxxxx col: ' . $as_column_count . ' row: ' . $as_row_count;
        $columns = 1;//number of column

        if ($as_row_count == 1) {
            //first row
            switch ($as_column_count) {
                case '1/1':
                    $columns = 3;
                    break;
            ...
            //row in row
            if ($as_column_count == '1/2') {
                $columns = 1;
            }

            if ($as_column_count == '1/3') {
         ...
         */
        if(as_global::$current_template == 'page-title-sidebar'){
            $as_page = get_post_meta($post->ID, 'as_page', true);

            //check for this page sidebar position
            if (!empty($as_page['as_sidebar_position'])) {
                $sidebar_position_pos = $as_page['as_sidebar_position'];
            } else {
                ...
                $sidebar_position_pos = as_util::get_option('tds_page_sidebar_pos');
            }
    ...
    static function get_featured_image_src($post_id, $thumb_type) {
        ...
        $as_temp_image_url = wp_get_attachment_image_src($attachment_id, $thumb_type);

        if (!empty($as_temp_image_url[0])) {
            return $as_temp_image_url[0];
        } else {
            return '';
        }
    }
    ...
    static function add_image_size_if_enabled($id, $x, $y, $crop) {
        if (as_util::get_option('tds_thumb_' . $id) != '') {
...
as_util::read_once_theme_settings();


class as_category2id_array_walker extends Walker {
    var $tree_type = 'category';
    var $db_fields = array ('parent' => 'parent', 'id' => 'term_id');

    var $as_array_buffer = array();
    ...
    function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
        $this->as_array_buffer[str_repeat(' - ', $depth) .  $category->name] = $category->term_id;
    }
...
if (!function_exists('as_is_as_mobile_plugin_active')) {
	function as_is_as_mobile_plugin_active() {
...
if (!function_exists('as_is_mobile_theme')) {
	function as_is_mobile_theme() {
